...
import com.quan.utility.dis_calculate
import org.apache.spark.sql.SparkSession
import org.apache.spark.sql.functions.col
/**
 * BHJ between two tables
 */
object BHJ {
  def main(args: Array[String]): Unit = {
    ...
    val resjoin= spark.sql("SELECT /*+ BROADCASTJOIN(t1) */ * FROM t1 right join t2 on t1.healpix = t2.healpix;")
      .withColumn("dis", dis_calculate.calculate(col("t1.ra"), col("t1.dec"), col("t2.ra"), col("t2.dec")))
      .filter($"dis" < 0.0000013888)

    resjoin.rdd.saveAsTextFile("./sparkdata/joinresout1/" )
    sc.stop()
  }
}
